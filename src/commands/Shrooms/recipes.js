...
module.exports = {
	data: new SlashCommandBuilder().setName('recipes').setDescription('Display all the posible recipes'),
...
const button = new ActionRowBuilder()
  ...
  msrms.forEach((row) =>{
    ...
    if(row.Show=='Mix'&&row.Consumable){
        msmrsEmb.addFields({name: row.Name+ row.Icon , value: row.Desc });
        frog++;

    }
    if(((frog%5)==0)&&row.Show=='Mix'&&row.Consumable){
      g++;
